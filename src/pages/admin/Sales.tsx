...
import { FiChevronDown, FiChevronUp, FiMail, FiPhone, FiMapPin, FiGlobe, FiHome, FiCheckCircle, FiXCircle, FiBox } from 'react-icons/fi';
    ...
    <div className="container flex flex-col gap-3 mx-auto p-6">
      ...
      {orders.map((order) => (
        ...
        >
          ...
          >
            <div className="mx-4 my-6 text-sm opacity-60 space-y-2">
              <PriceToast price={order.amount} className="text-5xl opacity-60 mb-2 pb-2 font-mono border-b" />
              <div className='flex gap-2 pb-1 border-b'>Products/<PriceToast price={Number(order.description.cartTotal)}/> </div>
              <div className='flex gap-2 pb-1 border-b'>Shipping/<PriceToast price={Number(order.description.shippingFees)}/> </div>
              <p></p>
              <p className="flex items-center gap-2"><FiMail /> Email: {order.owner || 'Not provided'}</p>
              <p className="flex items-center gap-2"><FiPhone /> Phone: {order.description.telephone || 'Not provided'}</p>
              <p className="flex items-center gap-2"><FiMapPin /> Address: {order.description.address || 'Not provided'}</p>
              <p className="flex items-center gap-2"><FiGlobe /> Country: {order.description.country || 'Not provided'}</p>
              <p className="flex items-center gap-2"><FiHome /> City: {order.description.selectedCity || 'Not provided'}</p>
              ...
              <div className="ml-6 space-y-1">
                ...
                {order.description.cart.map((item, index) => (
                  <div 
                  key={index}
                  className={`overflow-hidden [&>*]:text-[10px] flex items-center gap-2 ml-4 transition-all duration-300 ease-in-out`}
                    style={{
